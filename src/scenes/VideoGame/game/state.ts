...
export const restartGame = (state: GameState) => {
	const { level, score, enemySpawns, lives, shotCount, enemyCount, isGameOver } = initialState();
	Object.assign(state, {
		level,
		score,
		enemySpawns,
...
export function removeDeadEntities(state: GameState) {
	state.entities = state.entities.reduce<Entity[]>((array, item) => (item.dead ? array : [...array, item]), []);
}
